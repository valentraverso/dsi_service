...
export class CreditechService {
    ...
    async getPreapproval(dni: string, gender: "F" | "M", username: string, pass: string): Promise<any> {
        ...
        try {
            ...
            await page.goto(loginUrl, { waitUntil: 'networkidle2', timeout: 18000 });

            // 2. Esperar campos de formulario txtLogin y txtContrasenia
            await page.waitForSelector('#txtLogin, input[name="txtLogin"]', { visible: true, timeout: 10000 });
            await page.waitForSelector('#txtContrasenia, input[name="txtContrasenia"]', { visible: true, timeout: 10000 });

            await page.type('#txtLogin, input[name="txtLogin"]', username);
            await page.type('#txtContrasenia, input[name="txtContrasenia"]', pass);

            await page.waitForSelector('#btnIngresar, input[name="btnIngresar"]', { visible: true, timeout: 8000 });
            await page.click('#btnIngresar, input[name="btnIngresar"]');

            await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 18000 }).catch(() => {});
            ...
            await page.goto(pasos0Url, { waitUntil: 'networkidle2', timeout: 18000 });
            ...
            // 7. Parsear resultado (Aprobado o Rechazado)
